...
export const namedType = (
  ...
  const schemaValue = (name: string): ts.VariableStatement =>
    Node.Const({ name, isExport: true, value: JSON.stringify(schema) });
  ...
  const contextWithRef = namespace
    ? /**
       ...
       * prefix the consts with the namespace.
       */
      context.refs && schemaName in context.refs
      ? withIdentifier(
          withIdentifier(context, schemaValue(`${namespaceName}${name}Schema`)),
          Node.Const({ name: `${namespaceName}${fieldName}`, isExport: true, value }),
        )
      : withIdentifier(
          withIdentifier(context, schemaValue(`${name}Schema`)),
          Node.Const({ name: fieldName, isExport: true, value }),
        )
    : context;

  return document(withIdentifier(contextWithRef, type), Type.Referance(name));
};
